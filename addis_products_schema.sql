...
-- NOTE: If you WANT to enable RLS on the users table later, uncomment the
-- block below and re-run. Make sure ALL policies (including the anon read
-- policy) are created atomically to avoid breaking public page joins.
--
-- ALTER TABLE public.users ENABLE ROW LEVEL SECURITY;
-- CREATE POLICY "users_select_anon_public" ON public.users FOR SELECT TO anon USING (true);
-- CREATE POLICY "users_select_authed_read" ON public.users FOR SELECT TO authenticated USING (true);
...
-- CREATE POLICY "users_admin_select_all" ON public.users FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'));
-- CREATE POLICY "users_admin_update_all" ON public.users FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'));
